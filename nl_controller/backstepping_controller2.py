...
class BacksteppingController(Node):
    def __init__(self, name):
        ...
        self.declare_parameter('k3', 0.2)           # Backstepping参数3
        self.declare_parameter('gama', 0.0002)       # 自适应学习率
    ...
    def y_backstepping(self, y_d, y_d_dot, y_d_ddot, y_d_dddot, y_1, y_2, phi):
    ...
    def calculate_backstepping_attitude(self, pose, velo, traj_p, traj_v, traj_a, traj_j, euler_angles):
        """计算Backstepping姿态控制"""
        # 获取期望轨迹的导数
        traj_p_dot = traj_v
        traj_p_ddot = traj_a
        ...
        # 期望轨迹
        x_d, y_d, z_d = traj_p[0, 0], traj_p[1, 0], traj_p[2, 0]
        x_d_dot, y_d_dot, z_d_dot = traj_p_dot[0, 0], traj_p_dot[1, 0], traj_p_dot[2, 0]
        x_d_ddot, y_d_ddot, z_d_ddot = traj_p_ddot[0, 0], traj_p_ddot[1, 0], traj_p_ddot[2, 0]
        x_d_dddot, y_d_dddot, z_d_dddot = traj_p_dddot[0, 0], traj_p_dddot[1, 0], traj_p_dddot[2, 0]

        # 当前姿态角
        roll, pitch, yaw = euler_angles

        # Backstepping控制计算
        pitch_command = self.x_backstepping(x_d, x_d_dot, x_d_ddot, x_d_dddot, x_1, x_2, pitch)
        roll_command = self.y_backstepping(y_d, y_d_dot, y_d_ddot, y_d_dddot, y_1, y_2, roll)
    ...
    def controller_cb(self):
        ...
        traj_j = self.traj.jerk(self.traj_t)
        ...
        # 计算Backstepping姿态控制
        roll_cmd, pitch_cmd = self.calculate_backstepping_attitude(pose, velo, traj_p, traj_v, traj_a, traj_j, euler_angles)
