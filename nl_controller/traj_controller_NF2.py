...
class TrajController(Node):
    ...
    def calculate_desired_force(self, pose, velo, traj_p, traj_v, traj_a, sliding_gain, tracking_gain):
        ...
        F_sp = a_r - tracking_gain * s - adaptive_force / self.mass
    ...
    def update_adaptive_parameters(self, s, actual_thrust_vector, accel_world, delta_tk=None):
        ...
        R1 = 100 * np.eye(3)  # 测量噪声协方差

        # 计算实际扰动（测量值与模型预测的差异）
        disturbance = self.mass * (accel_world - np.array([[0], [0], [-self.gravity]]) - actual_thrust_vector)  # 3*1
        print("d",disturbance)

        # 自适应律，更新参数估计
        dot_a = -lambda_factor * self.current_hat.a - self.current_hat.P @ self.current_phi.T @ np.linalg.inv(R1) @ (self.current_phi @ self.current_hat.a - disturbance) + self.current_hat.P @ self.current_phi.T @ s
        self.current_hat.a = self.current_hat.a + delta_tk * dot_a

        a_norm = np.linalg.norm(self.current_hat.a, 'fro')
        if a_norm > self.norm_a_max:
            self.current_hat.a = self.current_hat.a / a_norm * self.norm_a_max

        dot_P = -2 * lambda_factor * self.current_hat.P + Q - self.current_hat.P @ self.current_phi.T @ np.linalg.inv(R1) @ self.current_phi @ self.current_hat.P
        self.current_hat.P = self.current_hat.P + delta_tk * dot_P
...
def main(args=None):
    try:
        ...
        node = TrajController("traj_controller_NF2")
        rclpy.spin(node)
